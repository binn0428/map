...
export default function Dashboard({ email, onLogout }: { email: string; onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    // 依 device 的 server_no 取得對應 Broker URL（selectedDevice 進入 deps 確保拿到最新值）
  ...
  // selectedDevice 整個物件進 deps，避免 server_no 為 null 時 closure 取到舊值
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [deviceId, mqttUser, mqttPass, serverNo, selectedDevice]);
  ...
  /* ── 修改設備名稱 ──────────────────────────────────────────────────────
     ...
     若輸入值與 device_name_initial 或 device_name 相同，視為「還原」，
     將 device_name_custom 清為 null，讓 displayName 自動回退到初始名稱。
  ...
  const handleRenameDevice = async () => {
    ...
    // 判斷是否為「還原原始名稱」：輸入值等於供裝初始名或 device_name
    const originalName = (selectedDevice.device_name_initial?.trim() || selectedDevice.device_name?.trim() || "");
    const isRestoring  = trimmed === originalName;

    // 若自訂名稱已相同，且不是還原動作，直接關閉
